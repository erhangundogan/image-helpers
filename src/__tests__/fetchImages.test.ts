...
describe('fetchImages', () => {
  const urls = ['https://images.unsplash.com/photo-1452570053594-1b985d6ea890?w=634&q=80'];
  const imageData = {
    width: 100,
    height: 100,
    time: 1617960172222,
  ...
  test('resolves images via Promise.allSettled', async () => {
    ...
    await expect(fetchImages(urls)).resolves.toEqual([imageData]);
    expect(spy).toBeCalledTimes(1);
  });

  test('resolves images via Promise.all', async () => {
    const spy = jest.spyOn(Promise, 'all');
    await expect(fetchImages(urls, { failEarly: true })).resolves.toEqual([imageData]);
    expect(spy).toBeCalledTimes(1);
  });

  test('resolves images with CORS', async () => {
    await expect(fetchImages(urls, { tryCors: true })).resolves.toEqual([imageDataWithCors]);
  });
});
